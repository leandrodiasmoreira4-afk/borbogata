...
import { AdminStockAdjustment } from "./admin-stock-adjustment";

type Tab = "visao" | "produtos" | "colecoes" | "pedidos";
type AdminProduct = AdminProductRecord;
type AdminOrder = { id:string; number:string; customer:string; value:number; paidCents:number; status:string; time:string };
...
const demoOrders:AdminOrder[] = [
  { id:"demo-1048", number:"#1048", customer:"Cliente demonstrativo", value:208.8, paidCents:20880, status:"Pago", time:"Demonstração" },
  { id:"demo-1047", number:"#1047", customer:"Cliente demonstrativo", value:359.8, paidCents:0, status:"Separando", time:"Demonstração" },
  { id:"demo-1046", number:"#1046", customer:"Cliente demonstrativo", value:139.9, paidCents:0, status:"Enviado", time:"Demonstração" },
];
...
export function AdminDashboard(){
  ...
  const loadData=useCallback(async()=>{
    ...
    const [productsResult,categoriesResult,ordersResult,collectionsResult,collectionProductsResult]=await Promise.all([
      ...
      supabase.from("orders").select("id,order_number,total_cents,status,created_at,customer_snapshot,payments(amount_cents,status)").eq("organization_id",orgId).order("created_at",{ascending:false}).limit(20),
    ...
    setOrders((ordersResult.data||[]).map((row)=>{const customer=row.customer_snapshot as {name?:string}|null;const payments=(row.payments||[]) as Array<{amount_cents:number;status:string}>;return {id:row.id,number:`#${row.order_number}`,customer:customer?.name||"Cliente",value:Number(row.total_cents)/100,paidCents:payments.filter((payment)=>payment.status==="paid").reduce((sum,payment)=>sum+Number(payment.amount_cents),0),status:row.status,time:new Intl.DateTimeFormat("pt-BR").format(new Date(row.created_at))}}));
  ...
  const canManageOrders=["owner","admin","orders"].includes(membershipRole||"");
...
function Overview({products,orders,setTab,canManageCatalog}:{products:AdminProduct[];orders:AdminOrder[];setTab:(tab:Tab)=>void;canManageCatalog:boolean}){const paidCents=orders.reduce((sum,order)=>sum+order.paidCents,0);const paidOrders=orders.filter(order=>order.paidCents>0).length;const lowStock=products.filter(product=>product.stock<=3).length;return <><div className="admin-heading"><div><p>Operação da loja</p><h1>Visão geral</h1></div>{canManageCatalog&&<button className="admin-primary" onClick={()=>setTab("produtos")}><Plus/>Novo produto</button>}</div><section className="metric-grid"><Metric label="Pagamentos aprovados" value={formatBRL(paidCents/100)} note="Dos 20 pedidos mais recentes" icon={<TrendingUp/>}/><Metric label="Pedidos" value={String(orders.length)} note="Até 20 pedidos recentes" icon={<ShoppingBag/>}/><Metric label="Ticket médio pago" value={formatBRL(paidOrders?paidCents/100/paidOrders:0)} note="Dos pedidos exibidos com pagamento aprovado" icon={<Users/>}/><Metric label="Estoque baixo" value={String(lowStock)} note="Até 3 unidades" icon={<PackageOpen/>}/></section><OrdersTable orders={orders}/></>}
...
function ProductsPanel({products,categories,organizationId,showForm,setShowForm,reload}:{products:AdminProduct[];categories:AdminCategory[];organizationId:string|null;showForm:boolean;setShowForm:(value:boolean)=>void;reload:()=>Promise<void>}){
  ...
  return <>
    ...
    {organizationId&&<AdminStockAdjustment products={products} organizationId={organizationId} reload={reload}/>}
...
function OrdersPanel({orders,organizationId,canManageOrders,reload}:{orders:AdminOrder[];organizationId:string|null;canManageOrders:boolean;reload:()=>Promise<void>}){return <><div className="admin-heading"><div><p>Acompanhe as vendas da loja</p><h1>Pedidos</h1></div></div>{storeConfig.shipping.correiosPostingMode==="manual-personal"&&<div className="collection-admin-tip"><PackageOpen/>Envios pelos Correios: após confirmar o pagamento, embale a peça, faça a pré-postagem como pessoa física e guarde o código de rastreio para informar ao cliente.</div>}<AdminOrders orders={orders} organizationId={organizationId} canManageOrders={canManageOrders} reload={reload}/></>}
