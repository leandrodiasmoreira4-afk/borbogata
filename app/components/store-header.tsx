...
export function StoreHeader() {
  ...
  return <>
    ...
    <header className="store-header">
      ...
      <Link href="/" className="brand" aria-label={`${storeConfig.name} — início`}><span className="brand-mark">{storeConfig.logoLetter}</span><span>{storeConfig.name.toUpperCase()}</span></Link>
    ...
    {open&&<div className="mobile-menu" role="dialog" aria-modal="true" aria-label="Menu"><div className="mobile-menu-top"><span className="brand">{storeConfig.name.toUpperCase()}</span><button className="icon-button" onClick={()=>setOpen(false)} aria-label="Fechar menu"><X/></button></div><nav><Link href="/produtos" onClick={()=>setOpen(false)}>Novidades</Link><Link href="/produtos" onClick={()=>setOpen(false)}>Vestidos</Link><Link href="/produtos" onClick={()=>setOpen(false)}>Conjuntos</Link><Link href="/produtos" onClick={()=>setOpen(false)}>Acessórios</Link><Link href="/admin" onClick={()=>setOpen(false)}>Painel administrativo</Link></nav></div>}
  </>;
}
